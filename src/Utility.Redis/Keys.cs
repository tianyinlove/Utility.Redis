...
namespace Utility.Redis
{
    ...
    public partial class RedisClient
    {
        ...
        public string Restore(string key, byte[] value, long ttlMilliSecond = 0)
        {
            ...
            try
            {
                using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
                {
                    return client.Restore(key, ttlMilliSecond, value).FromUtf8Bytes();
                }
            }
            catch (Exception err)
            {
        ...
        public bool ExpireAt(string key, DateTime timestamp)
        {
            if (string.IsNullOrWhiteSpace(key))
            {
                return false;
            ...
            using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
            {
                return client.ExpireAt(key, GetUnixTimestamp(timestamp));
            }
        }
        ...
        public List<string> Keys(string patten)
        {
            using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
            {
                return client.Keys(patten).Select(d => d.FromUtf8Bytes()).ToList();
            }
        }
        ...
        /// <returns></returns>
        public ScanResult Scan(ulong cursor, int count, string match)
        {
            using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
            {
                var scanResult = client.Scan(cursor: cursor, count: count, match: match);
                return new ScanResult { Cursor = scanResult.Cursor, Results = scanResult.Results.Select(d => d.FromUtf8Bytes()).ToList() };
            }
        }
        ...
        /// <param name="timestamp"></param>
        /// <returns>1 如果设置了过期时间 0 如果没有设置过期时间，或者不能设置过期时间</returns>
        public bool PExpireAt(string key, DateTime timestamp)
        {
            if (string.IsNullOrWhiteSpace(key))
            {
                return false;
            ...
            using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
            {
                return client.PExpireAt(key, (long)(timestamp.ToUniversalTime() - Epoch).TotalMilliseconds);
            }
        }
        ...
        public List<T> Sort<T>(string key,
                ...
                SerializerType serializerType = 0)
        {

            using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
            {
                var data = client.Sort(key, new SortOptions
                {
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public RedisKeyType Type(string key)
        {
            using (var client = (ServiceStack.Redis.RedisClient)ClientsManager.GetClient())
            {
                var type = client.Type(key);
        ...
        //KEYS,MIGRATE ,MOVE,OBJECT,RANDOMKEY,RENAME,RENAMENX,SCAN 不支持
    }
}
